...
- [x] Client-side settings screen (pause menu -> Options -> MCsocket) to edit host/port/auth/TLS and launch MCsocket actions without touching files
...
- Use the **Quick Actions** at the bottom of the screen to start/stop the WebSocket server or trigger `/ms recording` commands directly from the client (handy for singleplayer or when you have permission on a server).
