package com.djdnejk.mcsocket.controller;

import com.djdnejk.mcsocket.ModData;
import com.djdnejk.mcsocket.services.RecordingService;
import com.djdnejk.mcsocket.services.WebSocketService;
import com.mojang.brigadier.arguments.StringArgumentType;
import com.mojang.brigadier.context.CommandContext;
import me.lucko.fabric.api.permissions.v0.Permissions;
import net.fabricmc.fabric.api.command.v2.CommandRegistrationCallback;
import net.minecraft.server.command.CommandManager;
import net.minecraft.server.command.ServerCommandSource;
import net.minecraft.server.network.ServerPlayerEntity;
import net.minecraft.text.Style;
import net.minecraft.text.Text;
import net.minecraft.util.Formatting;

import static com.djdnejk.mcsocket.ModData.MOD_ID;

public class CommandController {

    public CommandController(WebSocketService wsService, RecordingService recordingService) {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
            dispatcher.register(CommandManager.literal("ms")
                // Uses the LuckPerms permission system to check if the player has the permission to use the command
                .requires(source -> source.hasPermissionLevel(2)
                    || Permissions.check(source, "command." + MOD_ID + ".ms", 2))
                .executes(this::sendHelp)
                .then(CommandManager.literal("help").executes(this::sendHelp))
                .then(CommandManager.literal("start").executes(ctx -> startWebSocket(ctx, wsService)))
                .then(CommandManager.literal("stop").executes(ctx -> stopWebSocket(ctx, wsService)))
                .then(CommandManager.literal("recording")
                    .then(CommandManager.literal("record").executes(ctx -> startRecording(ctx, recordingService)))
                    .then(CommandManager.literal("save").then(CommandManager.argument("name", StringArgumentType.word())
                        .executes(ctx -> saveRecording(ctx, recordingService))))
                    .then(CommandManager.literal("play").then(CommandManager.argument("name", StringArgumentType.word())
                        .executes(ctx -> playRecording(ctx, recordingService))))
                    .then(CommandManager.literal("list").executes(ctx -> listRecordings(ctx, recordingService)))
                    .then(CommandManager.literal("delete").then(CommandManager.argument("name", StringArgumentType.word())
                        .executes(ctx -> deleteRecording(ctx, recordingService))))
                    .then(CommandManager.literal("cancel").executes(ctx -> cancelRecording(ctx, recordingService))))));
    }

    private int sendHelp(CommandContext<ServerCommandSource> ctx) {
        Text helpText = Text.empty()
            .append(line("/ms - Main command"))
            .append(Text.literal("\n"))
            .append(line("/ms help - Show this help message"))
            .append(Text.literal("\n"))
            .append(line("/ms start - Start the WebSocket server"))
            .append(Text.literal("\n"))
            .append(line("/ms stop - Stop the WebSocket server"))
            .append(Text.literal("\n"))
            .append(line("/ms recording record - Start recording player actions"))
            .append(Text.literal("\n"))
            .append(line("/ms recording save <name> - Save the active recording"))
            .append(Text.literal("\n"))
            .append(line("/ms recording play <name> - Play a saved recording"))
            .append(Text.literal("\n"))
            .append(line("/ms recording list - List saved recordings"))
            .append(Text.literal("\n"))
            .append(line("/ms recording delete <name> - Remove a saved recording"))
            .append(Text.literal("\n"))
            .append(line("/ms recording cancel - Stop recording without saving"));

        ctx.getSource().sendFeedback(() -> ModData.prefixed(helpText), false);
        return 1;
    }

    private int startWebSocket(CommandContext<ServerCommandSource> ctx, WebSocketService wsService) {
        if (!wsService.isRunning()) {
            if (!wsService.tryToStart()) {
            ctx.getSource().sendFeedback(() -> Text.translatableWithFallback(
                "command." + MOD_ID + ".start_error",
                "An error occurred while starting the WebSocket server"), false);
            return 0;
        }
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("WebSocket server started")), false);
        } else {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("WebSocket server is already running")), false);
        }
        return 1;
    }

    private int stopWebSocket(CommandContext<ServerCommandSource> ctx, WebSocketService wsService) {
        if (wsService.isRunning()) {
            if (!wsService.tryToStop()) {
                ctx.getSource().sendFeedback(() -> Text.translatableWithFallback("command." + MOD_ID + ".stop_error",
                    "An error occurred while stopping the WebSocket server"), false);
                return 0;
            }
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("WebSocket server stopped")), false);
        } else {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("WebSocket server is not running")), false);
        }
        return 1;
    }

    private int startRecording(CommandContext<ServerCommandSource> ctx, RecordingService recordingService) {
        ServerPlayerEntity player = ctx.getSource().getPlayer();
        if (player == null) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Recording can only be started by a player.")), false);
            return 0;
        }

        if (recordingService.beginRecording(player)) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Recording started. Use /ms recording save <name> to persist.")), false);
            return 1;
        }

        ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Recording already running or could not start.")), false);
        return 0;
    }

    private int saveRecording(CommandContext<ServerCommandSource> ctx, RecordingService recordingService) {
        ServerPlayerEntity player = ctx.getSource().getPlayer();
        if (player == null) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Only players can save their recording.")), false);
            return 0;
        }

        String name = StringArgumentType.getString(ctx, "name");
        if (recordingService.saveRecording(player, name)) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Recording saved as '" + name + "'.")), false);
            return 1;
        }

        ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("No active recording to save.")), false);
        return 0;
    }

    private int playRecording(CommandContext<ServerCommandSource> ctx, RecordingService recordingService) {
        ServerPlayerEntity player = ctx.getSource().getPlayer();
        if (player == null) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Only players can play recordings.")), false);
            return 0;
        }

        String name = StringArgumentType.getString(ctx, "name");
        if (recordingService.playRecording(player, name)) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Playing recording '" + name + "'.")), false);
            return 1;
        }

        ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Recording '" + name + "' not found or already playing.")), false);
        return 0;
    }

    private int listRecordings(CommandContext<ServerCommandSource> ctx, RecordingService recordingService) {
        var names = recordingService.listRecordings();
        if (names.isEmpty()) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("No recordings saved.")), false);
            return 1;
        }
        ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Recordings: " + String.join(", ", names))), false);
        return 1;
    }

    private int deleteRecording(CommandContext<ServerCommandSource> ctx, RecordingService recordingService) {
        String name = StringArgumentType.getString(ctx, "name");
        if (recordingService.deleteRecording(name)) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Deleted recording '" + name + "'.")), false);
            return 1;
        }
        ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Recording '" + name + "' not found.")), false);
        return 0;
    }

    private int cancelRecording(CommandContext<ServerCommandSource> ctx, RecordingService recordingService) {
        ServerPlayerEntity player = ctx.getSource().getPlayer();
        if (player == null) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Only players can cancel recordings.")), false);
            return 0;
        }

        if (recordingService.cancelRecording(player)) {
            ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("Recording cancelled.")), false);
            return 1;
        }

        ctx.getSource().sendFeedback(() -> ModData.prefixed(Text.literal("No active recording.")), false);
        return 0;
    }

    private Text line(String content) {
        return Text.literal(content).setStyle(Style.EMPTY.withColor(Formatting.GRAY));
    }
}
